...
describe("server environment", () => {
  ...
  it("ignores non-string Astro metadata", () => expect(parseServerEnv({ ...base, ASSETS_PREFIX: { fallback: "/assets" } })).toEqual({
    appEnv: "development",
    appHostname: "localhost",
    supabaseUrl: "http://127.0.0.1:54321",
    supabaseKey: "sb_publishable_local_test",
  }));
  it("uses later string sources without letting undefined values erase earlier configuration", () => expect(parseServerEnv(
    base,
    { APP_ENV: undefined, APP_HOSTNAME: "platform.example.test" },
  ).appHostname).toBe("platform.example.test"));
